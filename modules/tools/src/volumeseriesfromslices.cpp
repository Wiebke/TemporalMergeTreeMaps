...
const ProcessorInfo VolumeSeriesFromSlices::getProcessorInfo() const { return processorInfo_; }
...
void VolumeSeriesFromSlices::process() { VolumeSourceSeries::process(); }
...
void VolumeSeriesFromSlices::fillCache() {
    ...
    for (size_t idx = 0; idx < numSlices; idx++, offset += offsetInc) {
        ...
        std::shared_ptr<Volume> pVolume =
            std::make_shared<Volume>(Volume(VolumeRAMSubSet::apply(vol, dimsSlice, offset)));
        // Pass meta data on
        pVolume->copyMetaDataFrom(*inVol.getData());
        pVolume->dataMap_ = inVol.getData()->dataMap_;

        // Adapt basis and offset
        vec3 volOffset = inVol.getData()->getOffset();
        mat3 volBasis = inVol.getData()->getBasis();

        const vec3 newOffset =
            volOffset + volBasis * (static_cast<vec3>(offset) / static_cast<vec3>(dims));

        mat3 newBasis = volBasis;
        vec3 dimRatio = (static_cast<vec3>(offsetInc) / static_cast<vec3>(dims));
