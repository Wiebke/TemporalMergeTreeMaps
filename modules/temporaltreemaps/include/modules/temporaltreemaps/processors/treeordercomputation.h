...
public:
    ...
    // Properties
public:
    /// Everything regarding optimization settings
    CompositeProperty propSettings;

    /// Maximum number of iterations
    IntSizeTProperty propIterationsMax;

    /// Settings regarding constraint extraction
    CompositeProperty propExtractionSettings;

    /// Aggregate Hierarchy constraints (meant for leaf only aggregation)
    TemplateOptionProperty<constraint::HierarchyAggregation> propAggregateHierarchy;

    /// Include merges and splits at upper leaves
    BoolProperty propUpperMergeSplit;

    /// Remove redundant constraints
    BoolProperty propRemoveRedundant;

	/// Show List of constraints
    BoolProperty propShowConstraints;
	/// List of constraints (leaf ids + constraints)
    StringProperty propConstraintsList;
	...
	/// Show actual order
	BoolProperty propShowOrder;

	/// Actual order with leaf indices
	StringProperty propActualOrder;

    /// Display current number of all constraints fulfilled
    StringProperty propFulfilledConstraintsTotal;

    /// Display statistics by level or in total
    BoolProperty propShowFulfilledConstraintsByLevel;

    /// Display statistics if hierarchy is reinforced (e.g. when using in NTG Rendering)
    BoolProperty propShowConstraintForReenforcedHierarchy;
...
protected:
    ...
    /// Rival constraints for non-hierarchy constraints
    ...
    size_t numConstraintsMergeSplit = 0;

    /// Total number of hierarchy constraints
    size_t numConstraintsHierarchy = 0;

    /// Max constraint level
    size_t maxConstraintLevel = 0;

    /// Max constraint size
    size_t maxConstraintSize = 0;
