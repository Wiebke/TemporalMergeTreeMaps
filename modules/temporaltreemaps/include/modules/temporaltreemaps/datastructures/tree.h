...
public:
    ...
    struct IVW_MODULE_TEMPORALTREEMAPS_API TNode {
...
public:
    ...
    */
    ...
    void getLeaves(const size_t nodeIndex, uint64_t startTime, uint64_t endTime,
    ...
    /// Determine if a node has immediate temporal successors given its index.
    bool hasTemporalSuccessors(const size_t nodeIndex) const {
        return getEdgesFrom(nodeIndex, edgesTime).size() > 0;
    }
    ...
    /// Get the immediate temporal predecessors of a node given its index using the reversely filled
    /// data structure
    ...
    bool hasTemporalPredecessorsWithReverse(const size_t nodeIndex) const {
        return getEdgesFrom(nodeIndex, reverseEdgesTime).size() > 0;
    }
...
public:
    ...
    */
    ...
    static uint64_t getPrevTimeFromTimes(const uint64_t time, const std::set<uint64_t>& times);

    uint64_t getPrevTime(const uint64_t time) const;

    static uint64_t getNextTimeFromTimes(const uint64_t time, const std::set<uint64_t>& times);

    uint64_t getNextTime(const uint64_t time) const;
    ...
    */
    ...
    TemporalTree aggregate(bool leavesOnly = false) const;
...
public:
    ...
    /// Type of tree aggregation
    TTreeAggregation aggregation = TTreeAggregation::Unknown;
...
struct DataTraits<kth::TemporalTree> {
    ...
    static Document info(const kth::TemporalTree& data) {
        std::string aggregateDescription;
        switch (data.aggregation) {
            case kth::TemporalTree::TTreeAggregation::Unknown:
                aggregateDescription = "unknown";
                break;
            case kth::TemporalTree::TTreeAggregation::LeafOnlyAggregation:
                aggregateDescription = "leaf-only";
                break;
            case kth::TemporalTree::TTreeAggregation::FullyDeaggregated:
                aggregateDescription = "no";
                break;
            case kth::TemporalTree::TTreeAggregation::FullyAggregated:
                aggregateDescription = "full";
                break;
            default:
                aggregateDescription = "unknown";
                break;
        ...
        oss << "Tree with " << data.nodes.size() << " nodes, " << data.edgesHierarchy.size()
            ...
            << data.nodes[0].endTime() << "] and " << aggregateDescription << " aggregation.";
        Document doc;
