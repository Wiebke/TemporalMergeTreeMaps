...
std::string orderToString(const TemporalTree& tree, const TemporalTree::TTreeOrder& order) {
    std::ostringstream stream;
    for (auto it = order.begin(); it != order.end(); it++) {
        auto leafIndex = *it;
        const auto& leafNode = tree.nodes[leafIndex];
        stream << leafIndex << "\t [" << leafNode.startTime() << ", " << leafNode.endTime() << "]";
        if (std::next(it) != order.end()) stream << std::endl;
    }
    return stream.str();
}
