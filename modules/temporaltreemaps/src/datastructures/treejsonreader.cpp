...
std::shared_ptr<TemporalTree> TemporalTreeJSONReaderNTG::readData(const std::string& filePath) {
    ...
    // Fill data values, can only be done now, since we need sucessor information for each node
    if (j.cend() != j.find("N")) {
        const json& jNodes = j["N"];
        for (json::const_iterator it = jNodes.cbegin(); it != jNodes.cend() && !bError; it++) {
            // Get the name, which is the key in the json
            const std::string NodeName = it.key();
            if (NodeNameToIndex.count(NodeName)) {
                const size_t& idxFromNode = NodeNameToIndex.at(NodeName);

                // Shorthand
                TemporalTree::TNode& ThisNode = pTree->nodes[idxFromNode];

                // Get the value, which is a json object containing time step (t), data (w), and
                // other unused info
                const json& jValue = it.value();
                const uint64_t Time = (jValue.find("t") != jValue.cend()) ? jValue["t"] : json(0);
                ...
                // If nodes has successors extend its time
                if (pTree->getTemporalSuccessors(idxFromNode).size() >= 1) {
                    auto itNextTime = std::next(times.find(Time));
                    if (itNextTime != times.end()) {
                        // To make it consistent with our data structure
                        ThisNode.values.emplace(*itNextTime, DataValue);
                    } else {
                        bError = true;
                    }
                }
    ...
    if (bError) {
        throw DataReaderException("Could not read nodes!",
                                  IvwContextCustom("TemporalTreeJSONReaderNTG"));
    }
    ...
    pTree->aggregation = TemporalTree::TTreeAggregation::FullyDeaggregated;
