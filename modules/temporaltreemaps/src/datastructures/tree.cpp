...
void TemporalTree::splitTemporaryLeaves() {
    ...
    for (size_t nodeIndex(1); nodeIndex < numNodes; nodeIndex++) {
        ...
        std::vector<size_t> childIndices = edgesHierarchy[nodeIndex];
        // Times at which this node has leaves
        std::set<uint64_t> hasChildren;

        for (auto leafIndex : childIndices) {
            ...
            for (; itStartTimeChild != itOneAfterEndTimeChild; itStartTimeChild++) {
                hasChildren.insert(*itStartTimeChild);
            }
        }

        auto itStartTimeNode = times.find(node.startTime());
        // distance is excluding the end, therefore
        const auto itEndTimeNode = std::next(times.find(node.endTime()));

        auto numTimeStepsNode = std::distance(itStartTimeNode, itEndTimeNode);
        // Are there times at which this node is a leaf
        if (numTimeStepsNode != int(hasChildren.size())) {
            ...
            for (auto it = itStartTimeNode; it != itEndTimeNode; it++) {
                ...
                const auto itIsLeaf = hasChildren.find(*it);
                bool isLeafNow = itIsLeaf == hasChildren.end();

                // Same state as before or very beginning
                if (stateBefore == 0 || (!isLeafNow && stateBefore == 1) ||
                    (isLeafNow && stateBefore == 2)) {
                    // Just extend the node
                    lastNode.values.emplace(*it, nodeValues[*it]);
                    // Was not a leaf before, now is a leaf
                }     /*else if (isLeafNow && stateBefore == 1) {
                ...
                else  // if (itIsLeaf != hasChildren.end() && stateBefore == 2))
                {
                    // Finish the old node
                    lastNode.values.emplace(*it, nodeValues[*it]);
                    newNodes.push_back(lastNode);
                ...
                stateBefore = itIsLeaf == hasChildren.end() ? 2 : 1;
            }
            // Add the final node
            newNodes.push_back(lastNode);
            ...
            if (newNodes.size() >= 2) {
                ...
                for (auto parent : parents) {
                    ...
                    uint64_t parentEndTime = nodes[parent].endTime();
                    // If this node has a sucessor, then it was only extended to ensure continuity
                    if (hasTemporalSuccessors(parent))
                        parentEndTime = getPrevTimeFromTimes(parentEndTime, times);

                    // Connect to all children that overlap
                    for (size_t index = 0; index < newNodes.size(); index++) {
                        uint64_t newNodeStartTime = newNodes[index].startTime();
                        uint64_t newNodeEndTime = newNodes[index].endTime();
                        if (newNodeStartTime != newNodeEndTime &&
                            hasTemporalSuccessors(newIndices[index]))
                            newNodeEndTime = getPrevTimeFromTimes(newNodeEndTime, times);
                        if (TNode::isOverlappingTemporally(parentStartTime, parentEndTime,
                                                           newNodes[index].startTime(),
                                                           newNodeEndTime)) {
                ...
                for (auto child : children) {
                    ...
                    uint64_t childEndTime = nodes[child].endTime();
                    if (childStartTime != childEndTime && hasTemporalSuccessors(child))
                        childEndTime = getPrevTimeFromTimes(childEndTime, times);
                    for (size_t index = 0; index < newNodes.size(); index++) {
                        uint64_t thisNodeEndTime = newNodes[index].endTime();
                        if (hasTemporalSuccessors(newIndices[index]))
                            thisNodeEndTime = getPrevTime(thisNodeEndTime);
                        const uint64_t overlapEnd = std::min(childEndTime, thisNodeEndTime);

                        if (TNode::isOverlappingTemporally(childStartTime, childEndTime,
                                                           newNodes[index].startTime(),
                                                           thisNodeEndTime)) {
        ...
        IVW_ASSERT(reverseEdgesHierachy.size() == nodes.size() - 1, "Someone does not have parent")
    }
}  // namespace kth
...
void TemporalTree::getLeaves(const size_t nodeIndex, uint64_t startTime, uint64_t endTime,
                             std::set<size_t>& Leaves) const {
    const TNode& node = nodes[nodeIndex];

    uint64_t nodeStartTime = node.startTime();
    uint64_t actualEndTime = node.endTime();
    // If this node has a sucessor, then it was only extended to ensure continuity
    if (nodeStartTime != actualEndTime && hasTemporalSuccessors(nodeIndex))
        actualEndTime = getPrevTime(node.endTime());
    if (!TNode::isOverlappingTemporally(startTime, endTime, nodeStartTime, actualEndTime)) {
        return;
    }

    auto it = edgesHierarchy.find(nodeIndex);
    // Called on a leaf, insert and return
    if (it == edgesHierarchy.end()) {
        Leaves.insert(nodeIndex);
        return;
    }

    // Use time of this node to detect overlap with children
    startTime = std::max(startTime, nodeStartTime);
    endTime = std::min(endTime, actualEndTime);

    for (const auto& childIndex : it->second) {
        // Is the child overlapping with the given time
        const auto& child = nodes[childIndex];
        uint64_t actualEndTimeChild = child.endTime();
        if (child.startTime() != actualEndTimeChild && hasTemporalSuccessors(childIndex))
            actualEndTimeChild = getPrevTime(child.endTime());
        if (TNode::isOverlappingTemporally(startTime, endTime, child.startTime(),
            ...
            if (edgesHierarchy.find(childIndex) == edgesHierarchy.end()) {
                ...
                uint64_t endTimeChild = std::min(endTime, actualEndTimeChild);
                getLeaves(childIndex, startTimeChild, endTimeChild, Leaves);
            }
        }
    }
}
...
size_t TemporalTree::depth(const size_t nodeIndex) const {
    ...
    // All parents will have the same distance to the root
...
size_t TemporalTree::depthWithReverse(const size_t nodeIndex) const {
    ...
    ivwAssert(parents.size() != 0, "A node does not have a parent");
    // All parents will have the same distance to the root
    return 1 + depthWithReverse(parents[0]);
}
...
void TemporalTree::deaggregate(TemporalTree& tree) const {
    ...
    tree.aggregation = TTreeAggregation::FullyDeaggregated;

    // This tree is already deggregated, return copy
    if (aggregation == TTreeAggregation::FullyDeaggregated) {
        tree = TemporalTree(*this);
        return;
    }
    ...
    for (auto node : nodes) {
        ...
        // If node is from previous aggregation it will have - to seperate the original node names
        std::string nodeName = node.name;
        std::vector<std::string> deaggregatedNodeNames;
        size_t start = 0;
        size_t end = nodeName.find("-");
        while (end != std::string::npos) {
            deaggregatedNodeNames.push_back(nodeName.substr(start, end - start));
            start = end + 1;
            end = nodeName.find("-", start);
        }
        deaggregatedNodeNames.push_back(nodeName.substr(start, end - start));
        // No "-" was every found and the vector just containts the entire name
        if (deaggregatedNodeNames.size() == 1) deaggregatedNodeNames.clear();
        // Sort by timestep suffix, if one exists
        std::sort(deaggregatedNodeNames.begin(), deaggregatedNodeNames.end(),
                  [](const std::string& a, const std::string& b) -> bool {
                      auto indexA = a.find("_");
                      size_t tA = 0;
                      if (indexA != std::string::npos) {
                          tA = stoi(a.substr(indexA + 1));
                      }
                      auto indexB = b.find("_");
                      size_t tB = 0;
                      if (indexB != std::string::npos) {
                          auto substr = b.substr(indexB + 1);
                          tB = stoi(b.substr(indexB + 1));
                      }
                      return tA < tB;
                  });
        ...
        for (auto itValue = values.begin(); itValue != itEnd; itValue++) {
            std::string name;
            if (deaggregatedNodeNames.size() > currentValueIndex)
                name = deaggregatedNodeNames[currentValueIndex];
            else {
                // If tree is already de-agreegrated, it will already have _ and timestep
                if (node.name.find("_") != std::string::npos) {
                    name = node.name;
                } else {
                    name = node.name + "_" + std::to_string(itValue->first);
                }
            }
            auto itNext = std::next(itValue);
            TNode newNode(name, {{itValue->first, itValue->second}});
            ivwAssert(itNext != values.end(), "Deaggregation: Next Value is the end.");
            // Extend node (will have another time step) with the current value (exclusive end)
            newNode.values[itNext->first] = itValue->second;
        ...
        // The node only has a single value or it ends here
        if (values.size() == 1 || successors.empty()) {
...
TemporalTree TemporalTree::aggregate(bool leavesOnly) const {
    // This tree already has the requested aggregation type, return copy
    if ((leavesOnly && aggregation == TTreeAggregation::LeafOnlyAggregation) ||
        !leavesOnly && aggregation == TTreeAggregation::FullyAggregated)
        return TemporalTree(*this);
    ...
    for (std::map<size_t, std::vector<size_t>>::const_iterator it = this->edgesTime.begin();
        ...
        if (it->second.size() == 1) {
            ...
            if (reverseEdges[it->second[0]].size() == 1) {
                // aggreage if both nodes are leaves, if everything should be aggregated, only
                // aggregate if both nodes are non-leaves
                /**bool firstIsLeaf = isLeaf(it->first);
                bool secondIsLeaf = isLeaf(it->second[0]);
                if ((firstIsLeaf && secondIsLeaf) || (!leavesOnly && !firstIsLeaf &&
                !secondIsLeaf))**/
                if (!leavesOnly || (isLeaf(it->first) && isLeaf(it->second[0])))
    ...
    for (size_t nodeIndex = 0; nodeIndex < this->nodes.size(); nodeIndex++) {
        ...
        if (nodeIndex != correspondence) {
            ...
            aggregatedNode.name += "-" + thisNode.name;
            // Add values
            for (auto itThisNode = thisNode.values.begin(); itThisNode != thisNode.values.end();
                 itThisNode++) {
                // First value of a node should overwrite, all other values should not
                if (itThisNode == thisNode.values.begin()) {
                    aggregatedNode.values.insert_or_assign(itThisNode->first, itThisNode->second);
                } else {
                    aggregatedNode.values.insert({itThisNode->first, itThisNode->second});
                }
            }
        }
    }
    ...
    aggregatedTree.aggregation =
        leavesOnly ? TTreeAggregation::LeafOnlyAggregation : TTreeAggregation::FullyAggregated;
...
uint64_t TemporalTree::getPrevTimeFromTimes(const uint64_t time, const std::set<uint64_t>& times) {
    auto it = times.find(time);
    if (it != times.begin()) {
        auto itPrev = std::prev(it);
        return *itPrev;
    }
    return std::max(static_cast<int>(*times.begin()) - 1, 0);
}

uint64_t TemporalTree::getPrevTime(const uint64_t time) const {
    // Use precomputed times if there are available
    if (!allTimes.empty()) return getPrevTimeFromTimes(time, allTimes);
    // Otherwise collect them first
    std::set<uint64_t> times;
    getTimes(0, times);
    return getPrevTimeFromTimes(time, times);
}

uint64_t TemporalTree::getNextTimeFromTimes(const uint64_t time, const std::set<uint64_t>& times) {
    auto it = times.find(time);
    auto itNext = std::next(it);
    if (itNext != times.end()) return *itNext;
    return (*times.rbegin()) + 1;
}

uint64_t TemporalTree::getNextTime(
    const uint64_t time) const {  // Use precomputed times if there are available
    if (!allTimes.empty()) return getNextTimeFromTimes(time, allTimes);
    // Otherwise collect them first
    std::set<uint64_t> times;
    getTimes(0, times);
    return getNextTimeFromTimes(time, times);
}
...
bool TemporalTree::checkConsistency() const {
    ...
    for (size_t component = 1; component < numComponents; component++) {
        ...
        if (aggregation == TTreeAggregation::FullyAggregated &&
            componentEdgesToParents[component].size() != 1) {
