...
void GetLayerOrder(const TemporalTree& Tree, const std::map<size_t, size_t>& LeafOrderMap,
    ...
    for (const size_t idxParent : LevelIndices) {
        ...
        Tree.getLeaves(idxParent, tMinParent, tMaxParent, LeavesOfParent);
        ...
        if (LeavesOfParent.size()) {
            ...
            for (const size_t idxLeaf : LeavesOfParent) {
                if (LeafOrderMap.count(idxLeaf) == 0) {
                    LogInfoCustom("LayerOrder", "Leaf " << idxLeaf << " not found! Size = "
                                                        << LeafOrderMap.size() << " Enable Split Leaves in Filter");
                    continue;
                }
            ...
            // We are a leaf
            if (LeafOrderMap.count(idxParent) == 0) {
                LogInfoCustom("LayerOrder", "Leaf " << idxParent
                                                    << " not found! Size = " << LeafOrderMap.size()
                                                    << " Enable Split Leaves in Filter");
                continue;
            }
