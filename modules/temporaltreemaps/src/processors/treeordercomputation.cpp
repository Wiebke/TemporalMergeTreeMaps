...
TemporalTreeOrderOptimization::TemporalTreeOrderOptimization()
    ...
    , propExtractionSettings("extractSettings", "Constraints")
    , propAggregateHierarchy("aggregateHierarchy", "Aggregate Hierarchy")
    , propUpperMergeSplit("upperMergeSplit", "Include Upper Merge/Split", true)
    , propRemoveRedundant("removeRedundant", "Remove Redundant Merge/Split", false)
    , propShowConstraints("showConstraints", "List Constraints", false)
    , propConstraintsList("constraintsList", "Constraints List", "")
    ...
    , propShowOrder("showOrder", "Show Order", false)
    , propActualOrder("actualOrder", "Order", "")
    , propFulfilledConstraintsTotal("fulfilledConstraints", "Total", "")
    , propShowFulfilledConstraintsByLevel("showByLevel", "Show Statistics By Level", true)
    , propShowConstraintForReenforcedHierarchy("reinforceHierarchy",
                                               "Show Statistics If Reinforcing Hierarchy", false)
    ...
    propSettings.addProperty(propExtractionSettings);
    propAggregateHierarchy.addOption("none", "None", constraint::HierarchyAggregation::None);
    propAggregateHierarchy.addOption("fullMatch", "Full Match",
                                     constraint::HierarchyAggregation::FullMatch);
    propAggregateHierarchy.addOption("mergeSplitMatch", "Merge/Split Match",
                                     constraint::HierarchyAggregation::MergeSplitMatch);
    propAggregateHierarchy.addOption("excludeAllEndsBegins", "Exclude All End/Begin",
                                     constraint::HierarchyAggregation::ExcludeAllEndsBegins);
    propExtractionSettings.addProperties(propAggregateHierarchy, propUpperMergeSplit,
                                         propRemoveRedundant, propShowConstraints,
                                         propConstraintsList);

    propConstraintsList.visibilityDependsOn(propShowConstraints,
                                            [](const auto& p) { return p.get(); });

    propShowConstraints.onChange([&]() { updateOutput(); });
    propConstraintsList.setSemantics(PropertySemantics::Multiline);
    ...
    propCurrentState.addProperties(propShowOrder, propActualOrder);
    propActualOrder.setSemantics(PropertySemantics::Multiline);
    propActualOrder.visibilityDependsOn(propShowOrder, [](const auto& p) { return p.get(); });
    propShowOrder.onChange([&]() { updateOutput(); });
    ...
    propCurrentState.addProperty(propShowFulfilledConstraintsByLevel);
    propShowFulfilledConstraintsByLevel.onChange([&]() { updateOutput(); });

    propCurrentState.addProperty(propShowConstraintForReenforcedHierarchy);
    propShowConstraintForReenforcedHierarchy.onChange([&]() { updateOutput(); });
...
void TemporalTreeOrderOptimization::fillStatistics(const ConstraintsStatistic& statistic) {
    ...
    std::string mergeSplit = std::to_string(statistic.numFulFilledMergeSplitConstraints()) + " / " +
                             std::to_string(numConstraintsMergeSplit);

    if (propShowFulfilledConstraintsByLevel) {
        mergeSplit = "";
        ...
        if (mergeSplit.length() == 0) {
            mergeSplit += "0/0";
        }
    }

    std::string hierarchy = std::to_string(statistic.numFulfilledHierarchyConstraints()) + " / " +
...
void TemporalTreeOrderOptimization::initializeResources() {
    ...
    extractHierarchyConstraints(pInputTree, propAggregateHierarchy, constraints,
                                numByLevelHierarchy);
    numConstraintsHierarchy = constraints.size();
    numByLevelMergeSplit.clear();
    rivalConstraints.clear();
    extractMergeSplitConstraints(pInputTree, !propUpperMergeSplit, propRemoveRedundant, constraints,
                                 rivalConstraints, numByLevelMergeSplit);
...
double TemporalTreeOrderOptimization::evaluateOrder(const TemporalTree::TTreeOrder& order,
    ...
    for (auto& constraint : constraints) {
        ...
        if (!isFulFilled(constraint, pInputTree, order, orderMap, constraints, rivals)) {
...
void TemporalTreeOrderOptimization::updateOutput() {
    ...
    if (propShowConstraintForReenforcedHierarchy) {
        ConstraintsStatistic statisticReinforced;
        const auto orderToUse = propOutputBestOrder ? bestState.order : currentState.order;
        // Expand order to the entire tree
        TemporalTree::TTreeOrderMap orderMap;
        treeorder::toOrderMap(orderMap, orderToUse);
        auto orderMapFull = treeorder::expandToFullTree(*pTreeIn, orderMap);
        // This construncted full order may break some merge/split constraint again and be
        // inconsistent when not all hierarchy constraints were satisfied (order on parents not
        // deducible) Here we use the order of parents to
        TemporalTree::TAdjacency edges = pTreeIn->edgesHierarchy;
        for (auto& pair : edges) {
            treeorder::sortNodesByOrder(orderMapFull, *pTreeIn, pair.second);
        }
        TemporalTree::TTreeOrder ordertoEvaluate;
        treeorder::orderAsDepthFirst(ordertoEvaluate, *pTreeIn, edges);
        float value = evaluateOrder(ordertoEvaluate, &statisticReinforced);
        fillStatistics(statisticReinforced);
        propObjectiveValue.set(value);
    } else {
    ...
    // Fill constraints
    if (propShowConstraints) {
        std::ostringstream stream;
        for (const auto& c : constraints) {
            stream << c;
        }
        propConstraintsList.set(stream.str());
    }

    if (propShowOrder) {
        propActualOrder.set(propOutputBestOrder
                                ? treeorder::orderToString(*pTreeIn, bestState.order)
                                : treeorder::orderToString(*pTreeIn, currentState.order));
    }
