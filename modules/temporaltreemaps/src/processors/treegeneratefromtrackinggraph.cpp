...
const ProcessorInfo TemporalTreeGenerateFromTrackingGraph::processorInfo_{
    ...
    "Tree Generate From Tracking Graph (NTG)",           // Display name
...
TemporalTreeGenerateFromTrackingGraph::TemporalTreeGenerateFromTrackingGraph()
    ...
    , propSamplingContColormap("samplingContColormap", "Continuous Colormap Sampling")
    , propUseContColormap("useContColorMap", "Use Continuous Colormap", false)
    , propContColormap("contColormap", "Continious Colormap",
                       TransferFunction({{0.0f, vec4(0.2f, 0.2f, 0.2f, 1.0f)},
                                         {1.0f, vec4(1.0f, 1.0f, 1.0f, 1.0f)}}))
    , propDataRange("dataRange", "Data Range", 0.0, 1.0, std::numeric_limits<double>::lowest(),
                    std::numeric_limits<double>::max(), 0.01, 0.0, InvalidationLevel::Valid,
                    PropertySemantics::Text)
    , propSampleUniformlyInValueRange("sampleIsoalues", "Sample IsoValues Uniformly")
    , propNoCompColor("noCompColor", "Non Component Color", vec4(1.0f), vec4(0.0f), vec4(1.0f),
                      vec4(0.1f), InvalidationLevel::InvalidOutput, PropertySemantics::Color)
    , propNearestNeighborInterpolation("nearestNeighborInterpolation", "Use NN interpolation",
                                       false)
    , propRelation("relation", "Relation")
    , propHierarchyLevelGroup("HierarchyLevelGroup", "Hierarchy")
    , propHierarchyColorLevelGroup("HierarchyLevelGroupColors", "Hierarchy Colors")
    ...
    , propActionAutomatically("actionAutomatically", "Start Tracking Automatically", false)
    ...
    addProperty(propSamplingContColormap);
    propSamplingContColormap.addProperty(propUseContColormap);
    propSamplingContColormap.addProperty(propContColormap);
    propSamplingContColormap.addProperty(propNoCompColor);
    propSamplingContColormap.addProperty(propDataRange);
    propSamplingContColormap.addProperty(propSampleUniformlyInValueRange);

    propSampleUniformlyInValueRange.onChange([&]() {
        const size_t nCurrentLevels = propHierarchyLevelGroup.size() - 1;

        auto minValue = propDataRange.get()[0];
        auto maxValue = propDataRange.get()[1];

        if (nCurrentLevels < 1) return;
        if (nCurrentLevels == 1) {
            auto singleProperty = (DoubleProperty*)propHierarchyLevelGroup[1];
            singleProperty->set((minValue + maxValue) / 2);
        }
        // If there are more than 2 levels include the borders
        auto dx = (maxValue - minValue) / (nCurrentLevels - 1);
        for (size_t i = 0; i < nCurrentLevels; i++) {
            auto currentProperty = (DoubleProperty*)propHierarchyLevelGroup[i + 1];
            currentProperty->set(minValue + i * dx);
        }
    });

    addProperty(propNearestNeighborInterpolation);

    propContColormap.visibilityDependsOn(propUseContColormap,
                                         [](const auto& p) { return p.get(); });
    propDataRange.visibilityDependsOn(propUseContColormap, [](const auto& p) { return p.get(); });
    propNoCompColor.visibilityDependsOn(propUseContColormap, [](const auto& p) { return p.get(); });
    ...
    addProperty(propHierarchyColorLevelGroup);
    ...
    propNumLevels.onChange([&]() {
        ...
        for (size_t i(nCurrentLevels); i < nDesiredLevels; i++) {
            DoubleProperty* ppropIsovalue = new DoubleProperty(
                "Isovalue" + std::to_string(i + 1), "Isovalue " + std::to_string(i + 1), 0.0,
            ...
            FloatVec4Property* ppropIsovalueColor = new FloatVec4Property(
                "ColorIsovalue" + std::to_string(i + 1),
                "Color for Isovalue " + std::to_string(i + 1), vec4(1.0f), vec4(0.0f), vec4(1.0f),
                vec4(0.1f), InvalidationLevel::Valid, PropertySemantics::Color);
            // Set tot hex color representation
            ppropIsovalueColor->setMetaData<IntMetaData>("representation", 2);
            propHierarchyColorLevelGroup.addProperty(ppropIsovalueColor, true);
        }

        // Delete old levels
        for (size_t i(nCurrentLevels); i > nDesiredLevels; i--) {
            propHierarchyLevelGroup.removeProperty(propHierarchyLevelGroup[i]);
            // colors does not have
            propHierarchyColorLevelGroup.removeProperty(propHierarchyColorLevelGroup[i - 1]);
        }
    });
    ...
    addProperty(propActionAutomatically);
}
...
void TemporalTreeGenerateFromTrackingGraph::UpdateCompColorMap() {
    auto& tfCont = propContColormap.get();
    auto& tf = propColormap.get();
    tf.clear();
    auto& range = propDataRange.get();
    ...
    auto sampleColor = [&](double value) -> vec4 {
        if (std::abs(value - range.x) < std::numeric_limits<double>::epsilon()) {
            return tfCont.sample(0.0 + std::numeric_limits<double>::epsilon());
        }
        if (std::abs(value - range.y) < std::numeric_limits<double>::epsilon()) {
            return tfCont.sample(1.0 - std::numeric_limits<double>::epsilon());
        }
        double normalizedValue = (value - range.x) / (range.y - range.x);
        return tfCont.sample(normalizedValue);
    };
    std::vector<double> Isovalues;
    GetIsovaluesSorted(Isovalues);
    // getNumber of IsoValues within dataRange
    const size_t NumIsovalues = Isovalues.size();
    // i will be mapped to double(i + 1) / double(NumIsovalues - 1 + 1);  // avoid zero
    const double dt = 1.0 / (NumIsovalues);
    double start = -0.5 * dt;
    LogProcessorInfo("Value span of one color " << dt * (range.y - range.x));
    addPoint(start, propNoCompColor.get());
    addPointAlmost(start + dt, propNoCompColor.get());
    for (size_t i = 0; i < NumIsovalues; i++) {
        auto& isoValue = Isovalues[i];
        vec4 color = sampleColor(isoValue);
        auto& relation = propRelation.get();
        // If comparison is greater and isoValue is smaller than the range, then the whole
        // domain will be greater, (analogously for comparison being smaller and isovalue being
        // larger than the range
        if (isoValue <= range.x && relation == GREATER) color = propNoCompColor;
        if (isoValue < range.x && relation == GREATEREQUAL) color = propNoCompColor;
        if (isoValue >= range.y && relation == SMALLER) color = propNoCompColor;
        if (isoValue > range.y && relation == SMALLEREQUAL) color = propNoCompColor;
        addPoint(start + (i + 1) * dt, color);
        addPointAlmost(start + (i + 2) * dt, color);

        if (i < propHierarchyColorLevelGroup.size()) {
            auto colorProperty = (FloatVec4Property*)(propHierarchyColorLevelGroup[i]);
...
void TemporalTreeGenerateFromTrackingGraph::UpdateMinMaxSlider(DoubleProperty& Slider) {
    const double& Val = Slider.getMinValue();
    Slider.setMinValue(Val < CurrentValueRange[0] ? Val : CurrentValueRange[0]);
    const double& MaxVal = Slider.getMaxValue();
    Slider.setMaxValue(MaxVal > CurrentValueRange[1] ? MaxVal : CurrentValueRange[1]);
}
...
#pragma omp parallel for schedule(static, 1) if (propParallel.get())
    ...
    OutTree.aggregation = TemporalTree::TTreeAggregation::FullyAggregated;
...
void TemporalTreeGenerateFromTrackingGraph::process() {
    ...
    // If continious color map should be used
    if (propUseContColormap.get()) UpdateCompColorMap();
    ...
    if (propNearestNeighborInterpolation.get())
        pOutVolume->setInterpolation(InterpolationType::Nearest);

    // Push it!
    portOutSegmentationExample.setData(pOutVolume);

    if (propActionAutomatically.get()) {
        GenerateNestedTrackingGraphs();
    }
}
