...
TemporalTreeOrderComputationSAEdges::TemporalTreeOrderComputationSAEdges()
    ...
    propAggregateHierarchy.onChange([&]() { initializeResources(); });
    propUpperMergeSplit.onChange([&]() { initializeResources(); });
    propRemoveRedundant.onChange([&]() { initializeResources(); });
...
void TemporalTreeOrderComputationSAEdges::initializeResources() {
    ...
    if (!mergeOrSplitFound) {
        initCase = HeuristicType::NoMergesOrSplits;
    } else {
        if (onlyMerges) {
            initCase = HeuristicType::MergesOnly;
        } else if (onlySplits) {
            initCase = HeuristicType::SplitsOnly;
        } else {
            initCase = HeuristicType::MergesAndSplits;
        }
    }
...
void TemporalTreeOrderComputationSAEdges::dfsTime(std::shared_ptr<const TemporalTree> tree,
    ...
    auto& edgesTime =
        initCase != HeuristicType::MergesOnly ? tree->edgesTime : tree->reverseEdgesTime;
    ...
    if (itEdgesTimes != edgesTime.end()) {
        ...
        for (auto sucessor : sucessors) {
            if (!visited[sucessor]) {
                if (initCase == HeuristicType::MergesOnly ||
                    initCase == HeuristicType::SplitsOnly) {
        ...
        for (auto child : children) {
            if (!visitedHierarchy[child]) {
                if (!visitedTime[child]) {
                    if (initCase != HeuristicType::MergesAndSplits) {
