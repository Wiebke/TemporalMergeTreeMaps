...

{
    ...
    propInitialConstraintOrder.onChange([&]() {
        restart();
        updateOutput();
    });
    ...
    propAggregateHierarchy.onChange([&]() {
        initializeResources();
        updateOutput();
    });
    propUpperMergeSplit.onChange([&]() {
        initializeResources();
        updateOutput();
    });
    propRemoveRedundant.onChange([&]() {
        initializeResources();
        updateOutput();
    });
...
void TemporalTreeOrderComputationHeuristic::findConflictingLeaves(
    ...
    for (size_t r(0); r < order.size(); r++) {
        ...
        if (constraint.leaves.find(leaf) != constraint.leaves.end()) {
            ...
            if (foundFirstLeaf) {
                const auto& leafNode = tree->nodes[order[r]];
                // Any overlap is important here, not only the one excluding endpoints
                if (!TemporalTree::TNode::isOverlappingTemporally(
                        leafNode.startTime(), leafNode.endTime(), constraint.startTime,
                        constraint.endTime)) {
                    nonConflictingAndConstraintLeaves.push_back(leaf);
                } else {
