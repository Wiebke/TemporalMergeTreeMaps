...
//      - Adaptive to Resize
// Change Log Wiebke/Tino
//      - Added functionality to load pre-computed layout in additional layout attribute
//      - Merging of laying paths
//      - Added custom color map

var NestedGraphLib = {

    // Some color maps from ColorBrewer (http://colorbrewer2.org/) + one custom colormap,
    // to be filled from outside the library
    colorMaps: {
        '3-class blue': ['#3182bd', '#6baed6', '#9ecae1'], // selected colors from 6-class blue
        '3-class green': ['#31a354', '#a1d99b', '#e5f5e0'], // actually 3-class green
    ...
    convertToDot: function(G, levels) {
        ...
        */
        ...
        for (nodeLabel in nodes) {
            ...
            if (!levels.includes(parseFloat(node.l))) {
                continue;
            }
        ...
        for (level in trackingGraphs) {
            if (!levels.includes(parseFloat(level)))
                continue;
            ...
            for (var startNode in trackingGraph) {
                ...
                for (i = 0, j = endNodes.length; i < j; i++) {
                    var weight = nodes[startNode].w;
                    if (!(endNodes[i] in nodes)) {
                        console.log("Node " + endNodes[i] + " is missing in nodes array. (" + startNode + " points to it.)");
                        //nodeString += nodeString += endNodes[i] + '[shape=box,fixedsize=true,height='+weight+'];\n';
                        //ranks[ nodes[startNode].t ] += ' '+ endNodes[i];
                    } else {
                        weight = weight / nodes[endNodes[i]].w;
    ...
    // Convert the first layer of G to the dot-format, but force the existing order
    convertToDotMinimal: function(G, levels) {
        var i, j, nodeLabel, time, level;
        var nodes = G.N;
        var trackingGraphs = G.ET;
        var nestingTrees = G.EN;
        ...
        */
        ...
        for (var timeStep in G.EN) {
            ...
            for (var nodeLabel in G.N) {
                ...
                if (parseFloat(node.l) == levels[0] && node.t == timeStep) {
    ...
    computeInitialLayout: function(G, levels) {
        var nodes = G.N;
        // Compute layout layerwise
        for (var level of levels) {
            var dotFormat = this.convertToDot(G, [level]);
            //// Creates layout with
    ...
    computeInitialLayoutMinimal: function(G, levels) {
        var dotFormat = this.convertToDotMinimal(G, levels);
        var nodes = G.N;
        //// Creates layout with
        //// graph scalefactor width height
        //// node name x y xsize ysize label style shape color fillcolor
    ...
    computeSlots: function(G, levels) {
        var nodes = G.N;
        ...
        for (var i = 0; i < levels.length - 1; i++) {
            ...
            for (var nodeLabel in nodes) {
                ...
                if (node.l === level) {
                    if (!("layout" in node)) {
                        console.log("Node " + nodeLabel + " does not have a layout.");
                        continue;
                    }
    ...
    drawNestedGraph: function(G, containerID, bForceNewLayout) {
        ...
        // Backup previous external layout?
        for (i in G.N) {
        ...
        for (i in G.N) {
            if (G.N[i].hasOwnProperty('layoutbackup') && !bForceNewLayout) {
                ...
                this.computeInitialLayoutMinimal(G, levels);
                ...
                this.computeInitialLayout(G, levels);
            }
            break;
        }
        ...
        // Compute Slots
        this.computeSlots(G, levels);

        // Draw each layer bottom to top
        var colors = this.colorMaps[this.currentColorMap];
        //console.log("Drawing with " + this.currentColorMap + " " + colors)
        var nColors = colors.length;
        //console.log("Got " + nColors + " colors");
        for (i = 0; i < levels.length; i++) {
            var layerColor = colors[i % nColors]
            //console.log("Using color" + layerColor + "for layer "+ i + ".")
            this.drawLayer(G, G.ET[levels[i]], layerColor, root);
            // Merge layer paths
            var combinedD = '';
            root.selectAll("path").each(function() {
                if (d3.select(this).attr("fill") == layerColor) {
        ...
        var zoomScale = Math.max(0.02,
            Math.min(10, 0.9 / Math.max(
                bb.width / containerSize.width, bb.height / containerSize.height)));


        svg.call(zoom.transform, d3.zoomIdentity.translate(
            containerSize.width / 2 - zoomScale * (bb.x + bb.width / 2),
            containerSize.height / 2 - zoomScale * (bb.y + bb.height / 2)
        ).scale(zoomScale));
    },
    ...
    drawLayer: function(G, trackingGraph, color, parent) {
        ...
        for (var startNode in trackingGraph) {
            ...
            for (var i = 0; i < endNodes.length; i++)
                if (!(endNodes[i] in nodes)) {
                    console.log("Node " + endNodes[i] + " is missing in nodes array.");
                } else {
